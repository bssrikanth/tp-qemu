...
def run(test, params, env):
    """
    QEMU test case to verify the Idle HLT Intercept feature and monitor idle-halt exits using ftrace.

    :param test: QEMU test object for logging and test control.
    :param params: Dictionary with test parameters (e.g., vm_secure_guest_type, url_cpuid_tool).
    ...
    def setup_ftrace(hlt_exit_reason):
        ...
        try:
            ...
            with open(os.path.join(trace_dir, "events/kvm/kvm_exit/filter"), "w") as f:
                f.write(filter_text)
            test.log.info("ftrace configured for kvm:kvm_exit with exit_reason == %s.", hlt_exit_reason)
    ...
    def cpuid_tool_build():
        ...
        for tool in ['make', 'gcc', 'tar']:
            if not shutil.which(tool):
                test.cancel(f"Build tool {tool} not found. Please install it (e.g., 'sudo apt install build-essential').")
        ...
        try:
            ...
            if result.exit_status != 0:
                test.fail("Installed cpuid tool failed to execute: {}".format(result.stderr.decode()))
            test.log.info("cpuid tool installed and verified: %s", result.stdout.decode().strip())
    ...
    if params.get("vm_secure_guest_type"):
        ...
        if secure_guest_type not in supported_secureguest:
            test.cancel("Testcase does not support vm_secure_guest_type %s"
                    % secure_guest_type)
    ...
    url_cpuid_tool = params.get('url_cpuid_tool',
    ...
    try:
        result = process.run('cpuid -1 -r -l 0x8000000A', shell=True, ignore_status=True)
        if result.exit_status != 0:
            test.cancel("Failed to execute cpuid command: {}".format(result.stderr.decode()))
    ...
    try:
        ...
        if "idle-halt" not in trace_output:
            ...
            test.log.info("Idle-halt exits detected in ftrace output:\n%s", trace_output)
    except Exception as e:
